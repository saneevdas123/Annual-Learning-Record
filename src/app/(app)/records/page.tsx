...
export default async function RecordsPage({
  ...
  function list(shown: typeof records) {
    if (shown.length === 0) {
      return <EmptyState title="Nothing in this tab" message="Try another status, or file a new record from a course." />;
    }
    return (
      <>
        ...
        <>
          <AppTabs
            ...
            panels={Object.fromEntries(
              TABS.map((t) => [t.key, list(t.key === 'all' ? records : records.filter((r) => r.status === t.key))])
            )}
          />
        </>
      )}
